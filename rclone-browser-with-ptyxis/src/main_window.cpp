  ...
  QObject::connect(
      ...
      this, [=](int code, QProcess::ExitStatus) {
        if (code == 0) {
          ...
          //QStringList lines = version.split("\n", QString::SkipEmptyParts); // This is the FIX
          QStringList lines = version.split("\n", Qt::SkipEmptyParts);
...
void MainWindow::rcloneConfig() {
      ...
      if (terminal.isEmpty()) {
        ...
        if (terminal.isEmpty()) {
          ...
          if (terminal.isEmpty()) {
            ...
            if (terminal.isEmpty()) {
              terminal = QStandardPaths::findExecutable("ptyxis");
              if (terminal.isEmpty()) {
  ...
  //p->setArguments(QStringList() << "-e" << terminalRcloneCmd); // This is the FIX
  p->setArguments(QStringList() << "--title" << "rclone config" << "-e" << "sh" << "-c" << terminalRcloneCmd);
...
void MainWindow::addStream(const QString &remote, const QString &stream) {
  ...
  //player->start(stream, QProcess::ReadOnly); // This is the FIX
